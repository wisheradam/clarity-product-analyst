...
"@
    ...
    Write-Host ""
    Write-Host "Running one Copilot prompt via PowerShell stdin..."
    Write-Host "Copilot stdout is written to a temp file to avoid the Windows -p quoting bug."
    Write-Host ""
    ...
    # IMPORTANT:
    # On Windows, some Copilot CLI versions retokenize multi-word values passed
    # through -p/--prompt. Feed the prompt through PowerShell's native pipeline
    # instead, and redirect stdout/stderr with PowerShell itself.
    $FullPrompt | & $copilot.Source @CopilotArgs 1> $StdoutPath 2> $StderrPath
