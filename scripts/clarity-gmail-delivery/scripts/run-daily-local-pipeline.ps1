...
$EmailPath = Join-Path $ProjectRoot "scripts\send-daily-email.mjs"
...
try {
    ...
    Assert-FileExists $EmailPath "Email sender"
    ...
    Run-Step "7. SEND DAILY EMAIL" {
        if ($ForceEmail) {
            node ".\scripts\send-daily-email.mjs" --force
        }
        else {
    ...
    Write-Host "Email       : PASS / SAFE-SKIP"
    Write-Host "AI Credits  : 0"
    Write-Host ""
